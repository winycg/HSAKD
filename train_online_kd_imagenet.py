...
def main_worker(gpu, ngpus_per_node, args):
    ...
    num_auxiliary_branches = len(ss_logits[0])
    ...
    if args.resume:
        print('load weights from: {}'.format(os.path.join(args.checkpoint_dir, str(model.__name__) + '.pth.tar')))
    ...
    def train(epoch, criterion_list, optimizer):
        ...
        ss_top1_num = [[0] * num_auxiliary_branches, [0] * num_auxiliary_branches]
        ss_top5_num = [[0] * num_auxiliary_branches, [0] * num_auxiliary_branches]
        class_top1_num = [[0] * num_auxiliary_branches, [0] * num_auxiliary_branches]
        class_top5_num = [[0] * num_auxiliary_branches,[0] * num_auxiliary_branches]
        top1_num = [0, 0]
        top5_num = [0, 0]
        total = 0
        ...
        for batch_idx, (input, target) in enumerate(trainloader):
            ...
            for i in range(len(logits)):
                loss_cls = loss_cls + criterion_cls(logits[i][0::4], target)
            loss_div = loss_div + criterion_div(logits[0], logits[1].detach())
            loss_div = loss_div + criterion_div(logits[1], logits[0].detach())

            for i in range(len(ss_logits)):
                for j in range(len(ss_logits[i])):
                    loss_cls = loss_cls + criterion_cls(ss_logits[i][j], labels)
            
            for i in range(len(ss_logits[0])):
                loss_div = loss_div + criterion_div(ss_logits[0][i], ss_logits[1][i].detach())
                loss_div = loss_div + criterion_div(ss_logits[1][i], ss_logits[0][i].detach())
            ...
            for i in range(len(ss_logits)):
                for j in range(len(ss_logits[i])):
                    top1, top5 = correct_num(ss_logits[i][j], labels, topk=(1, 5))
                    ss_top1_num[i][j] += top1
                    ss_top5_num[i][j] += top5
            

            class_logits = [[], []]
            for i in range(len(ss_logits)):
                for j in range(len(ss_logits[i])):
                    class_logits[i].append(torch.stack(torch.split(ss_logits[i][j], split_size_or_sections=4, dim=1), dim=1).sum(dim=2))


            multi_target = target.view(-1, 1).repeat(1, 4).view(-1)
            for i in range(len(class_logits)):
                for j in range(len(class_logits[i])):
                    top1, top5 = correct_num(class_logits[i][j], multi_target, topk=(1, 5))
                    class_top1_num[i][j] += top1
                    class_top5_num[i][j] += top5


            for i in range(len(logits)):
                logits[i] = logits[i].view(-1, 4, num_classes)[:, 0, :]
                top1, top5 = correct_num(logits[i], target, topk=(1, 5))
                top1_num[i] += top1
                top5_num[i] += top5
            total += target.size(0)

            print('Epoch:{}, batch_idx:{}/{}, lr:{:.5f}, Duration:{:.2f}, Top-1 Acc:{:.4f}'.format(
                epoch, batch_idx, len(trainloader), lr, time.time()-batch_start_time, (top1_num[0]/(total)).item()))
        ...
        for j in range(len(logits)):
            ss_acc1.append([round((ss_top1_num[j][i]/(total*4)).item(), 4) for i in range(num_auxiliary_branches)])
            ss_acc5.append([round((ss_top5_num[j][i]/(total*4)).item(), 4) for i in range(num_auxiliary_branches)])
            class_acc1.append([round((class_top1_num[j][i]/(total*4)).item(), 4) for i in range(num_auxiliary_branches)] + [round((top1_num[j]/(total)).item(), 4)])
            class_acc5.append([round((class_top5_num[j][i]/(total*4)).item(), 4) for i in range(num_auxiliary_branches)] + [round((top5_num[j]/(total)).item(), 4)])
    ...
    def test(epoch, criterion_cls, net):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, target) in enumerate(testloader):
                ...
                print('Epoch:{}, batch_idx:{}/{}, Duration:{:.2f}, Top-1 Acc:{:.4f}'.format(
                    epoch, batch_idx, len(testloader), time.time()-batch_start_time, (top1_num[0]/(total)).item()))
        ...
        return max([class_acc1[0][-1], class_acc1[1][-1]])
